...
def add_new_plant(new_plant: Plants):
    try:
        ...
        db.collection(FIRESTORE_COLLECTION_PLANTS).document(new_plant.name).set(
            new_plant.model_dump()
        )
...
@router.get(
    "/{plant_id}", response_model=Plants, dependencies=[Depends(verify_firebase_token)]
)
