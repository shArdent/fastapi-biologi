...
from schemas.my_plants import MyPlantSummary, MyPlantCreate, PaginatedMyPlantSummary, MyPlantUpdate, SuccessUpdatePlant, SuccessResponse, SuccessCreatePlant
from utils.verify_token import verify_firebase_token
from constants.collection_name import FIRESTORE_COLLECTION_USERS, FIRESTORE_COLLECTION_MY_PLANTS, FIRESTORE_COLLECTION_PLANTS, FIRESTORE_COLLECTION_DISEASES, FIRESTORE_DOCUMENT_METADATA
...
@router.post("/{user_id}/my-plants", response_model=SuccessCreatePlant, status_code=status.HTTP_201_CREATED)
def add_my_plant(user_id: str, plant_data: MyPlantCreate):
    try:
        ...
        if not plant_ref.get().exists:
            raise HTTPException(status_code=404, detail=f"Tanaman dengan ID '{plant_data.plant_id}' tidak ditemukan.")
        ...
        if plant_data.disease_id:
            ...
            if not disease_ref.get().exists:
                raise HTTPException(status_code=404, detail=f"Penyakit dengan ID '{plant_data.disease_id}' tidak ditemukan.")
...
def get_all_my_plants(
    ...
    search_nickname: Optional[str] = Query(None, description="Cari berdasarkan nickname (prefix search)")
):
    try:
        ...
        for doc in my_plants_docs:
            ...
            plant_name = plant_doc.to_dict().get('name', 'Tanpa Nama') if plant_doc.exists else "Data Tanaman Tidak Ditemukan"
            ...
            if disease_ref:
                ...
                if disease_doc.exists:
                    ...
                    summary["disease_name"] = disease_doc.to_dict().get('name', 'Tanpa Nama')

            summary_list.append(MyPlantSummary(**summary))

        next_last_doc_id = my_plants_docs[-1].id if len(my_plants_docs) == page_size else None
...
def delete_my_plant(user_id: str, my_plant_id: str):
    try:
        ...
        if not doc_ref.get().exists:
            raise HTTPException(status_code=404, detail=f"Tanaman dengan ID '{my_plant_id}' tidak ditemukan.")

        doc_ref.delete()

        meta_ref = db.collection(FIRESTORE_COLLECTION_USERS).document(user_id).collection(FIRESTORE_COLLECTION_MY_PLANTS).document(FIRESTORE_DOCUMENT_METADATA)
